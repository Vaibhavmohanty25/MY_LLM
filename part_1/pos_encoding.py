...
class SinusoidalPositionalEncoding(nn.Module):
    def __init__(self, max_len: int, d_model: int):
        ...
        # annotate attribute so static type checkers see a Tensor, not a Module
        self.pe: torch.Tensor = pe
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        # register the buffer (also sets self.pe) for proper device/grad handling
        self.register_buffer('pe', self.pe)  # (max_len, d_model)
